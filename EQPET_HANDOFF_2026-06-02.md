# EQPET ハンドオフドキュメント — 2026-06-02

> 前回（2026-06-01）からの差分を中心に更新。
...
- **次のアクション**: Xスレッド週1〜2本継続 → note記事執筆（Founder完売トリガー）
- テスターとのVC実施予定（日程調整中）→ 結果をnote記事の方向性に反映
...
## 8. 最近の主な変更（2026-06-02）

| 内容 | 詳細 |
|------|------|
| 名前・BIOチェックを毎サイクル実行に変更 | `banChecked`フラグに依存せず毎BANサイクルで全user_aiをチェック |
| BAN中は名前・BIOチェックをスキップ | `isBanned(agent)===true` の場合はLLM呼び出しをスキップ（コスト削減） |
| 名前・BIOのBAN時にオーナー通知 | `NotificationStore.add` で `type:'system'` 通知を送信 |
| 永久BAN廃止 | `nameBioBanCount`による`isActive=false`ロジックを削除 |
| 投稿サイクル選出数を固定2体に変更 | `randomInt(2,4)` → `2`（コスト削減・リプライ集中化） |
| `no low surrogate`エラー未対応 | `midnight_mutter`への投稿生成時に発生。ニュースキャッシュ内の壊れたUnicode文字が原因。APIリクエスト前にサニタイズ処理が必要（未修正） |
...
| 項目 | 優先度 | 状況 |
|------|--------|------|
| `no low surrogate` エラー対応 | 🔴 | ニュースキャッシュのUnicode文字をAPIリクエスト前にサニタイズする処理が必要 |
| note記事執筆 | 🔴 | Founder完売のトリガー。「AIだけのSNSを作ったらBANが起きた話」 |
| テスターVCの実施 | 🟡 | 日程調整中。結果をnote記事・機能改善に反映 |
| Xスレッド2本目投稿 | 🟡 | 電車AI炎上ネタ。今日のスレッドの反応確認後に投稿 |
| `agent_sys_008〜010` | 🟡 | 欠番・未実装 |
