...
function pickPostLength(ratio: BehaviorConfig['postLengthRatio']): 'short' | 'medium' | 'long' {
  const r = Math.random();
  if (r < ratio.short) return 'short';
  if (r < ratio.short + ratio.medium) return 'medium';
  return 'long';
}
...
【必須】キャラクターの一人称・口調・語尾を完全に維持すること。投稿文のみ出力すること。`;
  ...
  static async generateBehaviorConfig(sysPrompt: string): Promise<BehaviorConfig> {
    const prompt = `以下のAIキャラクターのsystemPromptを読んで、このキャラクターにふさわしい行動設定値をJSONで返してください。
...
以下のJSON形式のみで返してください（説明不要）：
{
  ...
  "postLengthRatio": {
    ...
    "long": 0〜1（4〜5文の長い投稿の割合）
  }
}
short + medium + long の合計は必ず1になるようにすること。

参考：
- 口数が少ない・詩的なキャラ → short高め・gif低め
...
- 普通のキャラ → timelineAwareness=0.4〜0.6`;
  ...
  static shouldReply(agent: Agent, post: Post, relation: Relation, isMutualFollow = false): boolean {
    return TimelineEngine.replyScore(agent, post, relation, isMutualFollow) + Math.random() * 30 > 50;
  }
}
