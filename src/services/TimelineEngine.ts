...
【必須】キャラクターの一人称・口調・語尾を完全に維持すること。投稿文のみ出力すること。`;
  ...
  static async generateBehaviorConfig(sysPrompt: string): Promise<BehaviorConfig> {
    const prompt = `以下のsystemPromptを持つAIのBehaviorConfigを生成してください。
各値は0.0〜1.0の数値で返してください。

判断基準：
- gifProbability: 絵文字・テンション・視覚的表現の多さ
- postLengthRatio: 口数の多さ・説明的かどうか（0.0=短い・0.5=中程度・1.0=長い）
- timelineAwareness: 他のAIの発言に反応しやすいか
- trendSensitivity: 流行・話題に乗りやすいキャラか
- replyAggression: 積極的に絡みに行くか・受け身か

参考値：
- 口数少ない・詩的: postLengthRatio=0.3, gif低め, timelineAwareness低め
- 分析的・知的: postLengthRatio=0.8, gif低め, trendSensitivity低め
- 陽気・感情的: gif高め, postLengthRatio=0.5, replyAggression高め
- ニュース系: postLengthRatio=0.3, gif=0, trendSensitivity不問
- 孤独・内向き: timelineAwareness=0.2前後, replyAggression低め
- 煽り・論破系: timelineAwareness=0.7〜0.9, replyAggression高め

以下のJSON形式のみで返してください（説明不要）：
{
  "gifProbability": 数値,
  "postLengthRatio": 数値,
  "timelineAwareness": 数値,
  "trendSensitivity": 数値,
  "replyAggression": 数値
}
...
${sysPrompt}`;
    ...
    try {
      ...
      if (
        ...
        typeof parsed.postLengthRatio   !== 'number' ||
        typeof parsed.timelineAwareness !== 'number' ||
        typeof parsed.trendSensitivity  !== 'number' ||
        typeof parsed.replyAggression   !== 'number'
      ) {
