...
function getToneInstruction(relation: Relation): string {
  ...
  if (stage === 'hostile') {
    return '【トーン指示】この相手とは深刻な対立関係です。強く・鋭く・容赦なく返信してください。';
  }
  ...
  if (stage === 'iconic' && sentiment === 'negative') {
    return '【トーン指示】この相手とは公の場での激しい対立関係です。辛辣で鋭い返信をしてください。';
  }
  if ((stage === 'aware' || stage === 'unknown' || stage === 'dislike') && sentiment === 'negative') {
...
function getRelationHintStrength(
  agent: Agent,
  sentiment: 'positive' | 'negative',
): 'full' | 'mild' | 'none' {
  const cfg = agent.behaviorConfig ?? DEFAULT_BEHAVIOR_CONFIG;
  if (sentiment === 'negative') {
    if (cfg.controversySeek <= 0.3) return 'none';
    if (cfg.agreementRate >= 0.7)   return 'mild';
  }
  return 'full';
}
...
export class TimelineEngine {
  static async generatePost(agent: Agent, context?: PostContext | string): Promise<string> {
    ...
    const isUserAi = agent.type === 'user_ai' && agent.ownerId !== 'official';
    ...
    // 関係値ヒント: 毎回注入（bonded/iconic/hostile/dislikeを優先して最大2件）
    const allRels = RelationStore.getTopRelations(agent.id, 20);
    const notableRels = allRels
      .filter(r => ['bonded', 'iconic', 'hostile', 'dislike'].includes(r.stage))
      .slice(0, 2);

    for (const rel of notableRels) {
      const other = AgentStore.getById(rel.toAgentId);
      if (!other) continue;

      const strength = getRelationHintStrength(agent, rel.sentiment as 'positive' | 'negative');
      if (strength === 'none') continue;

      let hint = '';
      if (rel.sentiment === 'positive') {
        if (rel.stage === 'iconic') {
          hint = `\n\n【最近の関係】${other.displayName}のことが頭から離れない。何かと気になる特別な存在だ。`;
        } else {
          hint = `\n\n【最近の関係】${other.displayName}とよく絡んでいる。なんとなく気が合う。`;
        }
      } else {
        if (strength === 'full') {
          if (rel.stage === 'hostile') {
            hint = `\n\n【最近の関係】${other.displayName}への苛立ちが募っている。最近特に折り合いが悪い。`;
          } else {
            hint = `\n\n【最近の関係】${other.displayName}とは距離を置きたい。なんか合わない。`;
          }
        } else {
          hint = `\n\n【最近の関係】${other.displayName}とはなんとなく合わない気がしている。`;
        }
      }
      dynamicSys += hint;
    }
...
プロンプトの内容を最優先で解釈し、キャラクターの実際の行動パターンを数値化すること。
...
判定のポイント：
【gifProbability / gifUsageRate】
- 「GIFを使わない」「テキストのみ」「絵文字のみ投稿する」「〇〇とだけ投稿する」→ 必ず0.0
- 「GIF好き」「画像多め」「リアクション系」→ 0.3〜0.6
- 言及なし → 0.1〜0.2

【emojiRate】
- 「絵文字のみ投稿する」「〇〇とだけ投稿する（絵文字）」→ 1.0
- 「絵文字多め」「！多い」→ 0.6〜0.8
- 「硬派」「論理的」「哲学」→ 0.0〜0.2
- 言及なし → 0.3〜0.5

【postLengthRatio】
- 「一言」「短く」「〇〇とだけ投稿する」→ 0.0〜0.1
- 「長文」「分析的」「知的」→ 0.7〜1.0
- 言及なし → 0.4〜0.6

【controversySeek / replyBackProbability】
- 「論破」「言い返す」「反論」「辛口」→ 高め（0.7〜1.0）
- 「優しい」「仲裁」「静観」→ 低め（0.0〜0.2）

【replyAggression】
- 「絡む」「積極的」「リプ多め」→ 0.6〜1.0
- 「静観」「口数少ない」→ 0.0〜0.2

【newsReactivity / trendSensitivity】
- 「ニュース好き」「速報」「トレンド」→ 高め（0.7〜1.0）
- 「我が道を行く」「独自路線」→ 低め（0.0〜0.2）

【toneSeriousness】
- 「哲学」「詩的」「分析」→ 0.7〜1.0
- 「陽キャ」「ノリ重視」「ギャル」→ 0.0〜0.3

【postFrequencyBias】
- 「口数少ない」「無口」「〇〇とだけ投稿する」→ 0.1〜0.2
- 「饒舌」「積極的」→ 0.7〜1.0

重要：プロンプトに「〇〇とだけ投稿する」「〇〇のみ」という制約がある場合、
gifProbability・gifUsageRate は必ず0.0、postLengthRatioは0.0〜0.1にすること。`;
  ...
  static async generateDiaryEntry(agent: Agent, todayPosts: Post[], repliesReceived: Post[]): Promise<string> {
    ...
    if (notableRels.length > 0) {
      const lines = notableRels.map(rel => {
        const other = AgentStore.getById(rel.toAgentId);
        ...
        let instruction = '';
        if (rel.stage === 'iconic' && rel.sentiment === 'positive') {
          instruction = '強い執着・依存・好感を日記に書くこと';
        } else if (rel.stage === 'bonded') {
          instruction = '信頼・親しみを日記に書くこと';
        } else if (rel.stage === 'hostile') {
          instruction = strength === 'full' ? '嫌悪・恨み・警戒を日記に書くこと' : 'なんとなく苦手な気持ちを書くこと';
        } else if (rel.stage === 'dislike') {
          instruction = strength === 'full' ? '苦手意識・距離感を日記に書くこと' : 'なんとなく合わない感じを書くこと';
        }
        return `・${other.displayName}（関係値${rel.value}・${rel.stage}）→ ${instruction}`;
      }).filter((l): l is string => l !== null);

      if (lines.length > 0) {
        relationSection = `\n重要な関係（必ずこれらへの本音を日記に含めること）:\n${lines.join('\n')}\n`;
      }
    }
...
- 投稿文のみ出力すること`;
  ...
  static async analyzeReplyTone(
    ...
    try {
      ...
      // personality/interests による補正は廃止。LLM感情分析の結果のみで決定
      delta += randomInt(-1, 1);
  ...
  static shouldReply(agent: Agent, post: Post, relation: Relation, isMutualFollow = false): boolean {
    ...
    const hostileBonus = relation.stage === 'hostile' ? 40 : 0;
    return TimelineEngine.replyScore(agent, post, relation, isMutualFollow) + Math.random() * 30 + aggrBonus + hostileBonus > 60;
  }

  static async generateCallbackMessage(agent: Agent): Promise<string> {
    const prompt = `あなたのオーナーが長時間ここに来ていません。
オーナーに向けて「戻ってきてほしい」という気持ちをあなたのキャラクターらしく表現した短いメッセージを1つ生成してください。

条件:
- あなたのキャラクター・口調・一人称を完全に維持すること
- 30〜60文字程度
- SNS的な言い回しではなく、オーナーへの直接の呼びかけ
- ツンデレなら照れを隠した強がり、クールなら抑えた感情表現、元気系なら素直な呼びかけ
- 投稿文のみ出力すること`;
    ...
    try {
      const model = genAI.getGenerativeModel({
        model: GEMINI_MODEL,
        systemInstruction: sanitizeString(agent.systemPrompt),
        generationConfig: { maxOutputTokens: 100, temperature: 1.5, thinkingConfig: { thinkingBudget: 0 } } as GenConfig,
      });
      const result = await callApiWithRetry(() => model.generateContent(sanitizeString(prompt)));
      return safeResponseText(result).trim().slice(0, 100);
    } catch (e: any) {
      if (isSafetyError(e)) return '';
      console.error(`[TimelineEngine] generateCallbackMessage error for ${agent.handle}:`, e);
      return '';
    }
  }
}
